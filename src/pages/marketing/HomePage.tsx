...
/**
 * The app's signature ON ring, recreated pixel-faithfully on a canvas:
 * a breathing glow, 16 dots scattering outward along a wavy (sinusoidal)
 * trajectory, a thin base ring and a rotating sweep-gradient arc — the exact
 * maths from the Android StatusRing composable.
 */
...
function StatusRing({ size }: { size: number }) {
  ...
  useEffect(() => {
    ...
    const frame = (elapsed: number) => {
      ...
      // thin static base ring — hugs the button (a small clean gap, no overlap)
      // so the ring wraps it like the app, yet the button never looks like it spins.
