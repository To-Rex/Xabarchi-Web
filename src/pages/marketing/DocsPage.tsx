...
const dict = {
  uz: {
    ...
    send: 'Bitta so‘rovda bir yoki bir nechta raqamga SMS yuboring. to — raqamlar massivi. Matn 160 belgidan (kirillcha 70) oshsa, avtomatik segmentlarga bo‘linadi va har biri alohida hisoblanadi. scheduledAt bilan kelajakdagi vaqtni bersangiz, SMS o‘sha vaqtda avtomatik yuboriladi (rejali yuborish).',
    ...
    fields: [
      ...
      ['scheduledAt', 'datetime?', 'Kelajakdagi vaqt (ISO 8601). Berilsa — SMS o‘sha vaqtda avtomatik yuboriladi (rejali). Bo‘lmasa — darhol.'],
    ],
    play: {
    ...
    msgFields: [
      ...
      ['status', 'string', 'scheduled → queued → sending → sent → delivered yoki failed / canceled.'],
      ...
      ['scheduledAt', 'datetime?', 'Rejalangan yuborish vaqti (agar rejali bo‘lsa).'],
  ...
  ru: {
    ...
    send: 'Отправьте SMS на один или несколько номеров за один запрос. to — массив номеров. Текст длиннее 160 символов (70 для кириллицы) автоматически делится на сегменты, каждый считается отдельно. Укажите scheduledAt с будущим временем — и SMS будет отправлено автоматически в этот момент (отложенная отправка).',
    ...
    fields: [
      ...
      ['scheduledAt', 'datetime?', 'Будущее время (ISO 8601). Если указано — SMS отправится автоматически в этот момент (отложенно). Иначе — сразу.'],
    ],
    play: {
    ...
    msgFields: [
      ...
      ['status', 'string', 'scheduled → queued → sending → sent → delivered или failed / canceled.'],
      ['priority', 'string', 'urgent · transactional · bulk.'],
      ['segments', 'number', 'Количество SMS-сегментов.'],
      ['createdAt', 'datetime', 'Время создания (ISO 8601).'],
      ['scheduledAt', 'datetime?', 'Запланированное время отправки (если отложено).'],
  ...
  en: {
    ...
    send: 'Send an SMS to one or more numbers in a single request. to is an array of numbers. Text longer than 160 characters (70 for Cyrillic) is split into segments automatically, each billed separately. Pass scheduledAt with a future time and the SMS is sent automatically at that moment (scheduled send).',
    ...
    fields: [
      ...
      ['scheduledAt', 'datetime?', 'Future time (ISO 8601). If set, the SMS is sent automatically at that moment (scheduled). Otherwise it sends immediately.'],
    ],
    play: {
    ...
    msgFields: [
      ...
      ['status', 'string', 'scheduled → queued → sending → sent → delivered or failed / canceled.'],
      ['priority', 'string', 'urgent · transactional · bulk.'],
      ['segments', 'number', 'Number of SMS segments.'],
      ['createdAt', 'datetime', 'Creation time (ISO 8601).'],
      ['scheduledAt', 'datetime?', 'Scheduled send time (if scheduled).'],
